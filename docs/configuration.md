...
Wake, watcher, away-mode, and Relay-specific state mechanics remain with their named scripts and reference sections; the inventory below names each path and its owner without restating those contracts.

### File inventory

Read this inventory before creating, changing, or deleting a `data/`, `state/`, or `config/` file that `AGENTS.md` does not name.
A parenthesized section number such as "(section 4)" below refers to that section of `AGENTS.md`.
Entries marked "never touch" are internal to their owner script and must not be edited, moved, or deleted by hand.

```
AGENTS.md            the always-loaded supervisor contract (CLAUDE.md is a real @AGENTS.md pointer to it)
...
The file inventory under "Operational home layout and state" routes the branch's runtime files to their format and lifecycle owners.
