...
Keep the routing rules authoritative where they live: `project-management`'s "Task intake" routes by natural-language `scope:` and keeps local-only projects with the main firstmate, and `AGENTS.md` keeps secondmates idle by default.
...
This section is the single owner of the secondmate sync and inherited-local-material propagation contract; `AGENTS.md` section 3 points here.
